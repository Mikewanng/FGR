...
class MQpath:
    ...
    def alg4(self,network,sdset,fthset,reqset,alpha=0,beta=1):
        ...
        for i in range(len(sdset)):
            ...
            pathset,dset,fiset,conset,thset,sumt,t=Qpath().alg1(c,sdset[i][0],sdset[i][1],fthset[i],reqset[i])
            pathfreed=[]#初始化路径自由度
            for path in pathset:
        ...
        while not self.q.empty():
            ...
            if Udtp().preudtppathc(network,cur[1],cur[2]):
                ...
                pathset,dset,fiset,conset,thset,sumt,t=Qpath().alg1(gcopy2,cur[0][0],cur[0][1],cur[0][2],cur[0][3]-self.sdth[dicsd[tuple([cur[0][0],cur[0][1]])]])
                pathfreed=[]#初始化路径自由度
                for i in pathset:
