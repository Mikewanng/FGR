...
class MQleap:
    ...
    def alg3(self,network,sdset,fthset,reqset):
        ...
        for i in range(len(sdset)):
            ...
            pathset,dset,fiset,conset,thset,sumt,t=Qleap().alg2(c,sdset[i][0],sdset[i][1],fthset[i],reqset[i])
            pathfreed=[]#初始化路径自由度
            for path in pathset:
        ...
        #    iteration_count += 1
            ...
            if Udtp().preudtppathc(network,cur[1],cur[2]):
                ...
                pathset,dset,fiset,conset,thset,sumt,t=Qleap().alg2(gcopy2,cur[0][0],cur[0][1],cur[0][2],cur[0][3]-self.sdth[dicsd[tuple([cur[0][0],cur[0][1]])]])
                #print(pathset)
                pathfreed=[]#初始化路径自由度
                for i in pathset:
