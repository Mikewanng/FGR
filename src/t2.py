...
#c作为自变量，单sd,数据保存于代码主文件夹下的txt文件中，运行次数count可以自定义。
#算法输入为拓扑，源，目的，保真度阈值，请求数量。
#算法返回值为：路径集，提纯决策集，保真度集，纠缠消耗集，吞吐量集，总吞吐量，寻路时间。

#算法变量设置
#总仿真次数
def SingleSdC(count=10,x=np.arange(100,1000,100),topology_fidelity_mode=0,nrof_requests=10000,alg1_mode=1,reqf=0.7,random_topology = 0,random_topology_mode=0,random_topology_nodes_num =5):
    """
    count=10
    reqf=0.7
    ...
    filename='Single_SDPair_vs_Capacity.txt'
    fp = open(filename, 'w')
    #先写入数据标题
    utput_label = 'fth      '
    if enable_Hspf == 1:
    ...
    for i in range(count):
        ...
        for j in range(len(x)):
            print("c: "+str(x[j]))
            g=Vtopo().changec(g,x[j])
            time_0=time.time()

            if enable_Hspf == 1:
                path,d,fi,con,th,sumt,times=Hspf().hspf(copy.deepcopy(g),sou,des,reqf,nrof_requests)
            ...
            time_2=time.time()
            if enable_alg2 == 1:
                path2,d2,fi2,con2,th2,sumt2,times2=Qleap().alg2(copy.deepcopy(g),sou,des,reqf,nrof_requests)
